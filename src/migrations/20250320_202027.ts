...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`header_nav_items_children_grandchildren\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`navigation_title\` text NOT NULL,
  	\`item_link_type\` text DEFAULT 'reference',
  	\`item_link_new_tab\` integer,
  	\`item_link_url\` text,
  	\`item_link_label\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`header_nav_items_children\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(
    sql`CREATE INDEX \`header_nav_items_children_grandchildren_order_idx\` ON \`header_nav_items_children_grandchildren\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`header_nav_items_children_grandchildren_parent_id_idx\` ON \`header_nav_items_children_grandchildren\` (\`_parent_id\`);`,
  )
  ...
  `)
  await db.run(
    sql`CREATE INDEX \`header_nav_items_children_order_idx\` ON \`header_nav_items_children\` (\`_order\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`header_nav_items_children_parent_id_idx\` ON \`header_nav_items_children\` (\`_parent_id\`);`,
  )
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`header_nav_items_children_grandchildren\`;`)
