...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`header_nav_items_children_grandchildren_order_idx\` ON \`header_nav_items_children_grandchildren\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`header_nav_items_children_grandchildren_parent_id_idx\` ON \`header_nav_items_children_grandchildren\` (\`_parent_id\`);`)
